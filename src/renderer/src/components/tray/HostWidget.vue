<template>
  ...
  >
    ...
    <div v-if="isGuestLockedOut" class="relative flex flex-col items-center">
      ...
      <div
        class="absolute -bottom-[12px] left-0 w-[280px] h-[2px] bg-white/5 overflow-hidden rounded-full"
      >
        <div
...
const remainingTime = computed(() => {
  ...
  const diff = lockoutUntil.value - currentTime.value
  return Math.max(0, diff / 1000)
})

const lockoutProgress = computed(() => {
  const total = 3000 // LOCKOUT_DURATION_MS
...
const handleHostLockout = (_event, data: { active: boolean; until: number }): void => {
  ...
  if (data.active && !timerInterval) {
    timerInterval = setInterval(() => {
      ...
      if (currentTime.value >= lockoutUntil.value) {
        stopTimer()
      }
    }, 50)
  }
...
onUnmounted(() => {
  stopTimer()
  window.electron.ipcRenderer.removeAllListeners('input:host-lockout')
})
